...
def listoffiles( pathin , pathout ):
	"""
...
#      dataset_options +=  ("     %s - %s \n" % (k, datasets[k].__name__))
    ...
    usage = ("usage: %prog <DATASET_PATH> <OUTPUT_PATH>\n\n"
...
def main():
	...
	data = load_imgs(r_dict['all_fnames'],(128,128), dtype='uint8')
	...
	dict = { 'num_cases_per_batch' : len(r_dict['all_labels'][r_dict['train_idxs']]) / nbatches,
	         ...
	         'num_vis'             : 128*128,
	         'data_mean'           : data[:,r_dict['all_labels'][r_dict['train_idxs']]].mean(axis=1,dtype='float32').reshape(128*128,1)
               }
